...
实现会先应用 `state_change`，再按声明顺序执行 `actions[]`。动作支持导航、弹窗展示/关闭、倒计时、正计时、视频控制、事件发送和自定义行为。倒计时与正计时只要求语义 `target`；编辑器不会再要求没有通用意义的倒计时参数。
...
- 同一次导航需要根据条件前往不同页面时，启用“根据条件跳转不同页面”，并为至少两个分支填写互斥、非空且唯一的条件及各自路由。
