...
- Treat `behaviors[]` as the single source of truth for page events. Keep state changes, popup presentation with caller-owned button callbacks, single-route or condition-branched navigation, countdown and count-up timers, and video control in the triggering behavior. Timer actions require only a semantic `target`; do not invent a generic countdown duration parameter when the design or app owns that value elsewhere.
