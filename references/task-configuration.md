...
- The current document version is `schema_version: 6`. Migration gives every item on an existing `done` page status `done` and creates its initial `accepted_baseline`; items on other legacy pages start `todo`. Migrating schema v5 converts deterministic popup result flows into typed presentation callbacks while preserving ambiguous fragments for manual resolution.
...
- `state_change` references a state on the interaction's source page. It runs before all `actions[]`, whose declaration order is the execution order. Action types are `navigate`, `present_popup`, `dismiss_popup`, `start_countdown`, `stop_countdown`, `play_video`, `pause_video`, `stop_video`, `emit_event`, and `custom`. Countdown and video actions require `actions[].target`; `start_countdown` also requires `parameters.duration_seconds`. Event and custom actions require `name`.
...
- `condition`, when present, is a non-empty implementation expression. When one trigger has several possible results, use mutually exclusive behavior conditions.
...
- `navigate` parameters carry destination input as non-empty runtime expressions for destination-creating routes.
...
- `start_countdown.parameters.duration_seconds` configures the countdown duration as a non-empty runtime expression. Other countdown and video actions do not accept parameters unless a future action-specific contract says otherwise.
