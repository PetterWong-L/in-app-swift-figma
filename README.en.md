...
The skill includes a local web application for editing `InAppFigma.yaml`, so you do not need to maintain a complex YAML document by hand. It listens only on `127.0.0.1`, uses a random token for the current session, and writes only the current project's `InAppFigma/InAppFigma.yaml`.

![InAppFigma web editor overview](docs/images/editor-overview.png)

The interface has four primary areas:

- **Top toolbar**: switch languages, reload the disk configuration, validate, preview normalized YAML, save manually, and stop the local server.
- **Configuration outline**: manage modules and pages, entry pages, delivery profile, parallelism, and explicitly approved system UI visual-gate exceptions.
- **Page editor**: edit the current page through the Page, Data, Behaviors, States, and Status tabs.
- **Flow canvas**: inspect pages, design states, navigation, state changes, and popup callbacks as a live graph.

### Page and task editing

The outline organizes configuration as module to page. The Page tab shows deterministic Swift filenames derived from `title`; Data manages shared mock contracts; States manages each Figma visual state; and Status exposes the `claim`, `complete`, `fail`, `block`, `requeue`, and `amend` lifecycle actions.

Every state, behavior, and popup structure has its own `todo | in_progress | done` status. When a completed page is edited, the web application does not overwrite its acceptance silently. It lists the affected tasks and saves the amendment atomically only after confirmation.

### Behaviors and popup callbacks

The Behaviors tab keeps the trigger, state change, and ordered actions in one behavior card. The example below configures a confirmation popup: the calling page supplies the title, body, button text, and each button callback. A button dismisses its concrete popup before executing the caller's state change and actions.

![Behavior and popup callback configuration](docs/images/editor-behaviors.png)

Cards expand and collapse independently and can be copied to other pages. Cards with validation errors open automatically, and each issue is attached to its owning field instead of being presented only as an opaque YAML path.

### Flow canvas

The flow canvas is generated from the configuration; no manual diagram editing is required. It displays:

- Every Figma state owned by a page.
- In-page state edges produced by `state_change`.
- Navigation edges for `push`, `sheet`, `full_screen`, `back`, and `dismiss`.
- A distinct popup instance for every `present_popup` occurrence and the continuation of each button callback.
- Module entry points, page status, and unreferenced popup templates.

Use zoom, pan, and full-screen controls for larger flows. Selecting a page heading also selects that page in the outline. Figma previews load lazily so a long task does not fetch all remote content at once.

![Full-screen page flow canvas](docs/images/editor-flow-canvas.png)

### Auto-save and conflict protection

Valid edits save automatically after roughly 800 milliseconds without further input. Edits made during a save are queued for the next save; validation failures keep the current draft; and an external disk change pauses auto-save until you reload or reconcile the newer version. Selecting Stop ends only the local server and does not change page status.
...
├── docs/images/              # Web editor screenshots used by the READMEs
